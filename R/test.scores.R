test.scores <- function (scores, listw, nsim = 999) 
{
  alter <- ifelse(scores$values > 0, "greater", "less")
  vec2test <- 1:dim(scores$vectors)[2]
  moran.w <- apply(cbind(vec2test), 1, function(x) moran.mc(scores$vectors[, 
                                                                           x], listw = listw, nsim = nsim, alternative = alter[x]))
