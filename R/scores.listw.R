scores.listw <- function (listw, echo = FALSE, MEM.autocor = c("all", "positive", 
                                               "negative")) 
{
  ...
  eq0 <- apply(as.matrix(res$values/max(abs(res$values))), 
               1, function(x) identical(all.equal(x, 0), TRUE))
  if (sum(eq0) == 0) {
    stop(" Illegal matrix: no null eigenvalue")
  }
  if (echo) {
    cat(paste("vector number", which(eq0), "corresponding to null eigenvalue is removed", 
              "\n"))
  }
